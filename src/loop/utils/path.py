...
def _initial_ignore_rules(folder: Path) -> IgnoreRules:
    ...
    rules = {filename: [] for filename in IGNORE_FILENAMES}
...
def _is_ignored(path: Path, is_directory: bool, rules: IgnoreRules) -> bool:
    ...
    if is_directory and path.name == ".git":
        return True

    agent_decision = _ignore_decision(path, is_directory, rules[".agentignore"])
    if agent_decision is not None:
        return agent_decision
    return _ignore_decision(path, is_directory, rules[".gitignore"]) is True
